...
| **R-FOYER-10** | La **vue staff** présente les présences sur une **période** (par défaut une semaine à partir de la date choisie), sous forme de cartes par jour. Pour chaque jour et chaque résidence, elle affiche le nombre de personnes **au foyer** (vert) et **sorties** (rouge) ; un détail au clic liste les habitantes avec leur statut. La population de référence = résidentes + invitées rattachées à la résidence. |
...
| **R-EVT-05** | La **visibilité** d'un événement peut être ciblée par **résidence**, **étage** et/ou **chambre**. Une habitante voit l'événement si elle correspond à l'un de ces critères. |
