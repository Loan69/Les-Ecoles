...
-- ════════════════════════════════════════════════════════════════════════════
--
-- Extrait le 2026-08-24 (après P0 à P3) de la base du Foyer des Écoles :
--   pg_dump --schema-only --schema=public --no-owner
--
-- Il absorbe le socle précédent ET toutes les migrations p2*/p3*, désormais
-- dans archive/. Aucune donnée, aucune valeur propre à un foyer.
--
-- Pour monter un foyer neuf, TROIS fichiers — voir docs/ouvrir-un-foyer.md :
--   1. ce fichier
--   2. supabase/storage-branding.sql   (le bucket vit dans le schéma `storage`,
--                                       que --schema=public ne capture PAS)
--   3. supabase/seed.sql
--   puis  node scripts/foyer-nouveau.mjs
--
-- Trois retouches par rapport à la sortie brute de pg_dump, toutes nécessaires
-- pour que le fichier passe dans l'éditeur SQL de Supabase :
--   · méta-commandes \restrict / \unrestrict retirées (elles ne valent que dans psql) ;
--   · CREATE SCHEMA public → IF NOT EXISTS ;
--   · bloc ALTER DEFAULT PRIVILEGES retiré (voir la note en fin de fichier).
--
-- Cible : PostgreSQL 17.
...
--
-- Name: est_super_admin(); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.est_super_admin() RETURNS boolean
...
CREATE TABLE public.invitations (
    ...
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT invitations_place_role_chk CHECK ((((role = 'super_admin'::text) AND (place_id IS NULL)) OR ((role = 'residente'::text) AND (place_id IS NOT NULL)))),
    CONSTRAINT invitations_role_check CHECK ((role = ANY (ARRAY['residente'::text, 'super_admin'::text]))),
    CONSTRAINT invitations_statut_check CHECK ((statut = ANY (ARRAY['envoyee'::text, 'acceptee'::text, 'expiree'::text, 'annulee'::text])))
);


--
-- Name: COLUMN invitations.place_id; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.invitations.place_id IS 'Place attribuée. NULL = invitation de super-administratrice, sans chambre (cf. role).';
...
CREATE POLICY "app_settings: ecriture gestion" ON public.app_settings TO authenticated USING (
CASE
    WHEN starts_with(key, 'foyer_'::text) THEN public.est_super_admin()
    ELSE ((public.mon_niveau('repas'::text) >= 3) OR (public.mon_niveau('absences'::text) >= 3) OR (public.mon_niveau('comptes'::text) >= 3))
END) WITH CHECK (
CASE
    WHEN starts_with(key, 'foyer_'::text) THEN public.est_super_admin()
    ELSE ((public.mon_niveau('repas'::text) >= 3) OR (public.mon_niveau('absences'::text) >= 3) OR (public.mon_niveau('comptes'::text) >= 3))
...
--
-- Name: app_settings app_settings: identite lisible publiquement; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "app_settings: identite lisible publiquement" ON public.app_settings FOR SELECT TO anon USING (starts_with(key, 'foyer_'::text));
...
--
-- Name: FUNCTION est_super_admin(); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.est_super_admin() FROM PUBLIC;
GRANT ALL ON FUNCTION public.est_super_admin() TO anon;
GRANT ALL ON FUNCTION public.est_super_admin() TO authenticated;
GRANT ALL ON FUNCTION public.est_super_admin() TO service_role;
...
--
-- pg_dump émettait ici 24 instructions ALTER DEFAULT PRIVILEGES visant les rôles
-- postgres et supabase_admin. L'éditeur SQL de Supabase tourne en tant que
-- `postgres`, qui ne peut pas modifier les privilèges par défaut d'un autre rôle :
-- ERROR 42501, permission denied to change default privileges.
--
-- Elles sont de toute façon redondantes : Supabase configure exactement ces
-- privilèges sur tout projet neuf. Les retirer ne change rien au résultat, et
-- rend le socle exécutable d'un seul tenant.
--
