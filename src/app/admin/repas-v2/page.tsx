...
import { CalendarDays, Table2, Scale, Soup, Moon as MoonIcon, CalendarCheck } from "lucide-react";
...
type DayResidence = { dejeuner: PersonneDetail[]; diner: PersonneDetail[] };
...
export default function AdminRepasV2Page() {
  ...
  // Détail d'un jour : listes de personnes qui mangent, par résidence et service
  const getDayDetail = useCallback(
    (dateKey: string): Record<string, DayResidence> => {
      const res: Record<string, DayResidence> = {};
      ...
      presences.forEach((p) => {
        ...
        if (p.service === "dejeuner") res[residence].dejeuner.push(person);
        else if (p.service === "diner") res[residence].diner.push(person);
      });
      return res;
    },
    [residences, presences, absences, peopleById, comptaResidence]
  );
    ...
    <div className="min-h-screen bg-gradient-to-br from-yellow-50 to-white py-10 px-4 sm:px-6">
                  ...
                  <div key={date} className="rounded-2xl border-2 border-gray-100 bg-white shadow-sm p-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {residences.map((r) => {
                        ...
                        const tile = (label: string, service: Service, ppl: PersonneDetail[], cls: string) => {
                          const notes: Record<string, string> = {};
                          ppl.forEach((person) => {
                            const pres = presences.find((x) => x.user_id === person.id && x.date === date && x.service === service);
                            const opt = pres ? optionsById.get(pres.option_id) : undefined;
                            if (opt) notes[person.id] = opt.label;
                          });
                          return (
                            <button
                              onClick={() => setListModal({ title: `${label} — ${r.label} · ${formatJourLong(date)}`, people: ppl, notes })}
                              className={`flex flex-col items-center rounded-xl py-2 transition cursor-pointer ${cls}`}
                              title="Voir la liste"
                            >
                              <span className="text-[10px] font-bold uppercase">{label}</span>
                              <span className="text-lg font-black">{ppl.length}</span>
                            </button>
                          );
                        };
                        return (
                          <div key={r.value} className="border border-gray-100 rounded-xl p-3">
                            ...
                            <div className="grid grid-cols-2 gap-2">
                              {tile("Midi", "dejeuner", d.dejeuner, "bg-orange-50 hover:bg-orange-100 text-orange-900")}
                              {tile("Soir", "diner", d.diner, "bg-blue-50 hover:bg-blue-100 text-blue-900")}
                            </div>
                          </div>
                        );
                      })}
            ...
            {residences.map((r) => (
              <div key={r.value} className="bg-white shadow-sm border border-gray-200 rounded-xl p-6">
                ...
                <table className="min-w-full border text-sm bg-white">
                  ...
                  <tbody>
                    {(comptaByResidence[r.value] ?? []).map((row) => (
                      <tr key={row.person.id} className="border-b">
                        <td className="p-2">
                          {row.person.nom} {row.person.prenom}
                          {row.person.isInvite && <span className="ml-1 text-[10px] text-blue-500">(invitée)</span>}
                        </td>
                        <td className="text-center p-2">{row.dejeuner}</td>
                        <td className="text-center p-2">{row.diner}</td>
                        <td className="text-center p-2 font-semibold text-amber-800">{row.total}</td>
                      </tr>
                    ))}
                    {(comptaByResidence[r.value]?.length ?? 0) === 0 && (
                      <tr><td colSpan={4} className="p-3 text-gray-400 italic text-center">Aucune inscription sur la période.</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
      ...
      {tableOpen && (
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50 px-4" onClick={() => setTableOpen(false)}>
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-6xl max-h-[85vh] overflow-y-auto p-5" onClick={(e) => e.stopPropagation()}>
